...
module.exports = {
  name: "list badges", //the command name for the Slash Command
  slashName: "list-badges", //the command name for the Slash Command
  category: "Utility",
  aliases: [], //the command aliases [OPTIONAL]
  description: "See a list of badges with numbers of users in it.", //the command description for Slash Command Overview
  cooldown: 1,
  ...
  slashRun: async (client, interaction) => {
    try {
      ...
      interaction.guild.members.fetch().then((fetchedMembers) => {
        const embedFields = Object.entries(UserFlags).reduce(
          (fields, [flagKey, flagName]) => {
            const flagCount = fetchedMembers.filter((member) =>
              member.user.flags.toArray().includes(flagKey)
            ).size;
            if (flagCount > 0) {
              fields.push(
                `<:_:${
                  client.allEmojis[flagKey]
                }> ${flagCount.toString()} People`
              );
            }
            return fields;
          },
