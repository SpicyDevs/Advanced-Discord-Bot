const {
  ...
  module.exports = {
    name: "lock", // the command name for the Slash Command
    slashName: "lock", // the command name for the Slash Command
    category: "Moderation",
    aliases: [], // the command aliases [OPTIONAL]
    description: "Sets the channel to read only for everyone who is not an Admin or Moderator", // the command description [OPTIONAL]
    cooldown: 1,
    ...
    options: [
        {"Channel": { name: "channel", description: "The channel to Lock", required: false }},
      ...
      try {
        ...
        channel = interaction.channel || interaction.options.getChannel("channel");
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(`Channel \`${channel.name}\` (${channel.id}) \n\nLocked by ${interaction.member.user} (${interaction.member.user.tag})`);
    ...
    messageRun: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
        try {
          ...
          const embed = new EmbedBuilder()
            .setTitle("Channel Locked Successfully!")
            .setDescription(`Channel \`${channel.name}\` (${channel.id}) \n\nLocked by ${message.author} (${message.author.tag})`);
