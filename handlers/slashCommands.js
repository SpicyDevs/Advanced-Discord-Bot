    ...
    readdirSync("./commands/").forEach((dir) => {
      if (lstatSync(`./commands/${dir}`).isDirectory()) {
        ...
        if (cmdSetup && cmdSetup.Folder) {
          ...
          const slashCommands = readdirSync(`./commands/${dir}/`).filter(
          ...
          for (let file of slashCommands) {
            ...
            if (pull.slashName && pull.description) {
              subCommand.addSubcommand((subcommand) => {
                subcommand
                  .setName(String(pull.slashName).toLowerCase())
                    ...
                    if (
                      ...
                      subcommand.addStringOption((op) =>
                        op
                          ...
                          .setAutocomplete(
                            option.String.autocomplete ? true : false
                          )
                      );
                    } else if (
                      option.Channel &&
                      ...
                      const choices = option.StringChoices.choices.map((c) => ({
                      ...
                      subcommand.addStringOption((op) =>
                        op
                          ...
                          .setAutocomplete(
                            option.IntChoices.autocomplete ? true : false
                          )
                          .addChoices(
                      ...
                      option.Attachment.description
                    ) {
                      subcommand.addAttachmentOption((op) =>
                        op
                          .setName(
                            String(option.Attachment.name)
                              .replace(/\s+/g, "_")
                              .toLowerCase()
                          )
                          .setDescription(option.Attachment.description)
                          .setRequired(option.Attachment.required)
                          .setAutocomplete(
                            option.Attachment.autocomplete ? true : false
                          )
                      );
                    } else {
                      console.log(
                        `A Option is missing the Name or/and the Description of ${pull.slashName}`
                      );
                    }
                  }
                }
              ...
              if (option.User && option.User.name && option.User.description) {
                ...
                Command.addStringOption((op) =>
                  op
                    ...
                    .setDescription(option.String.description)
                    .setRequired(option.String.required)
                    .setAutocomplete(option.String.autocomplete ? true : false)
                );
              } else if (
                option.Channel &&
                option.Channel.name &&
                ...
                Command.addStringOption((op) =>
                  op
                    ...
                    .setRequired(option.StringChoices.required)
                    .setAutocomplete(
                      option.StringChoices.autocomplete ? true : false
                    )
                    .addChoices(
      ...
      if (config.loadSlashsGlobal) {
        client.application.commands
          ...
          .then((slashCommandsData) => {
            ...
            console.log(
              `${slashCommandsData.size} slashCommands ${
                `(With ${
                  slashCommandsData.map((d) => d.options).flat().length
                } Subcommands)`.green
              } Loaded for all: ${`All possible Guilds`.underline}`.brightGreen
            );
            console.log(
              ...
              guild.commands
                ...
                .then((slashCommandsData) => {
                  ...
                  console.log(
                    ...
                    } Loaded for: ${`${guild.name}`.underline}`.brightGreen
                  );
                })
                .catch(console.error);
            } catch (e) {
